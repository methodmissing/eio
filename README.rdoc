...
implementations of the Reactor pattern (Eventmachine) where I/O requests and callbacks are always
submitted and handled on the reactor thread. This library thus naturally fits into event driven
applications and exposes a file descriptor that can wake up an event loop when readable and execute
callbacks for completed requests on the reactor thread as well.

Event loop integration is however much closer to the Proactor pattern. The I/O multiplexer in a Reactor
implementation merely notifies of file descriptor state changes - a handler is still responsible for
reading or writing data on the reactor thread. Callbacks for file system and other blocking system calls
wrapped by this library receive results as arguments - there's nothing else to do. In other words, the
Reactor pattern asynchronously notify of state changes, but act on those synchronously, on the Reactor
thread, which incurs some processing overhead.
...
I/O, the goal is increased throughput and not necessarily a faster per request guarantee. The goal is to
serve more clients with the same or less infrastructure without degrading quality of service.
...
* Best results with I/O bound work on disks / volumes with variable performance characteristics.
